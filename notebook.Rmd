...
```{r, group_by_lm}
# training_data_supervised <- group_by(training_data_supervised, drug)

# l_models <- dlply(training_data_supervised, "drug", function(training_data_supervised) lm(response ~ . - cellline, data = training_data_supervised))

# training_data %>%
#   group_by(drug) %>%
#   do(models = lm(response ~ . - cellline, data = .))

# training_data = training_data_supervised %>% 
#                   group_by(drug) %>%
#                   do(models = lm(response ~ . - cellline, data = .))

training_data %>% group_by(drug) %>% do(models = lm(response ~ . - cellline, data = .))
```
