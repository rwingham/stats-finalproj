...
# models <- training_data %>% group_by(drug) %>% do(model = lm(response ~ . - cellline, data = .))
# models <- training_data %>% selet(-cellline) %>% group_by(drug) %>% do(model = lm(response ~ ., data = .))

```

```{r}
models <- training_data %>% group_by(drug) %>% do(models = lm(response ~ V1 + V2 + V3 + V4 + V5 + V6 + V7 + V8 + V9 + V10 + V11 + V12 + V13 + V14 + V15 + V16 + V17 + V18 + V19 + V20 + V31 + V32 + V33 + V34 + V35 + V36 + V37 + V38 + normal_like + luminal + claudin_low + basal, data = .))
```
