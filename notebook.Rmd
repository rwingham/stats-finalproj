...
```{r, checks}
remove_rows <- which(training_set_results$cellline == setdiff(training_set_results$cellline, expression$cellline))
training_set_results <- training_set_results[-remove_rows,]
...
training_data <- inner_join(training_set_results,
                            ...
                            by = "cellline")
training_data <- gather(training_data, "drug", "response", 2:13)
# test_data <- subset(test_data, select=colnames(training_data[1:25]))
# test_data <- subset(test_)
...
bagged <- training_data %>%
            ...
            group_by(drug) %>%
            nest() %>%
            mutate(model = map(data, ~ bagging(response ~ ., data = ., ns = 25)))
...
I'm not getting a full mapping quite...
...
for(i in seq_along(bagged$drug)){
  for(j in seq_along(test_data$cellline)){
    ...
    if(length(row) == 0){
      match_drug <- which(outcome$drug == drug)
      if(length(match_drug) == 0){
        print(paste('drug', drug))
      }
      else  print(paste('cellline', test_data$cellline[j]))
    }
    rows <- c(rows, row)
  }
    # print(paste('id ', row))
...
# missing 6, 15, 24, 42, 51, 60, 69, 78, 87, 105, 


sort(unique(as.numeric(rows)))
```