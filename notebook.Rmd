...
Now that we can see that we have no missing data, we can evaluate types.  
...
Now  all of the data we imported is stored in the correct type for the purposes of analysis in `R`, so we can  go on to check for normalization in the gene expression data.  To do that, we evaluate the quantiles for each cell line, since we're not actually interested in what the actual values but in how much they vary.
...
While we can see that there is a change in the histogram showing the variance as we approach variance values of 5, we will cloose a much lower threshold due to our method of dimension reduction.  The first step is determining the eigen values and eigen vectors from the co-variance matrix.  Numerical solvers are fairly good, however, calculating the covariance between every gene involves making $n^2$ co-variance calculations.  Since we're starting with `r length(expression)` genes, that would result in  `r length(expression)`$^2$ variance calculations (where each feature has 25 values from the training data), so we need to slim it down somewhat.
...
At this point we have `r length(features_of_interest)` genes we are interested in, and given that we have `r nrow(training_expressions)` observations in our training set, this feature space is still far too large to do analysis on its own. So we now perform a PCA where we look at doing dimension reduction.  When performing a PCA, we take the eigen values of the covariance matrix.  Each eigen value corresponds to the error that would be introduced should we exclude the corresponding eigen vector and lessen the dimension by 1. While we can do the calculation manually by multiplying the data-frame training_expressions_pruned by our eigen vectors to get the resulting eigen values, we elected to use the `prcomp` library.
...
To determine how much of a dimension reduction we can do without introducing too much error, we plot the error, which is the square root of the eigen values.

```{r, pca_error_plot}
plot(sqrt(pca$sdev), type="l", xlab="Eigen Value Index", ylab="Sqrt Error")
```
The first plot shows what we would expect, the eigen-values which are the squre of the error are decreasing, but we effectively need at least 24 of the eigen-vectors to accurately account for the variance in our 25 samples.  This result is not unexpected given our very small sample size ($n = $`r nrow(training_expressions)`).  To me, this indicates that ideally, for a PCA, we would have more samples to better account for all the variance.  So we decide we will make use of all 25 of our eigen-vectors

```{r pca_selection}
expressions_pca <- pca$x
test_expressions_pca <- cbind(select(test_expressions, cellline), predict(pca, test_expressions_pruned))
```

We ended up going down to `r ncol(expressions_pca)` dimensions, and given our sample size, that's far more managable. The next step is to great our training and test data structures that we can test over.  In order to minimize repeating ourselves in our code, we use some tivyverse joining functions, and restructure the data using the gather function.
...
First we create our bagging function, which given n observations will make n samples with replacement as many times as we want (defaulting to 500 times), calculate the prediction parameters, and then average them out.

```{r, linear_regression_and_bagging}
...
Amazingly enough, the linear regression model didn't do too poorly.  At the time of submission, it was not the worst submssion made (though it was the second worst), with a score of 0.51.  The linear regression model did not take into account the subtype of the tumor (due to the limintation of input parameters), and since we were taking samples with replacements, the `lm` was throwing up warnings of having dependent samples, which is why I had to implement the `suppressWarnings` method.

In short, linear regression is not a great modeling tool when you have a highly dimensional feature space but very few samples ($ n \lt\lt p$).

# Random Forests

The next model type we used was random forests.  Random forests lends itself really nicely to this data set, as it has no problem handling high-dimensional with very small sample sizes.  Even though we reduced the expression set data down to 38 dimensions through the PCA, that's still more than the number of training samples we have to work with.  
