...
Importing the data was fairly straight forward, there were good descriptors provided.
...
The only oddity is that some of the provided files expected the user to make reference of row names, which tidyverse does not make use of.  We worked around this by importing those files using `read.table` and then using the tibble command `rownames_to_column`.  
