...
```{r dumb_bagging}
iterations <- 5000
training_data_slim <- subset(training_data, select=-c(drug, cellline))[1:25,]

predictions <- foreach(m=1:iterations, .combine=cbind) %do% {
  training_bag_set <- sample_n(training_data_slim, size=nrow(training_data_slim), replace=TRUE)
  # train_pos <- 1:nrow(training_data_slim) %in% training_positions
  suppressWarnings(lm_fit <- lm(response ~ ., data=training_bag_set, singular.ok=TRUE))
  suppressWarnings(predict(lm_fit, newdata=test_data))
}
...
```{r bagging_function}
bagging <- function(training, testing, drug, iterations=1000)
{
  training_data_slim <- filter(training, drug == drug)
  training_data_slim <- subset(training_data_slim, select=-c(drug))
  predictions <- foreach(m=1:iterations, .combine=cbind)  %do% {
    training_bag_set <- sample_n(subset(training_data_slim, select=-c(cellline)), size=nrow(training_data_slim), replace=TRUE)
    suppressWarnings(lm_fit <- lm(response ~ ., data=training_bag_set, singular.ok=TRUE))
    suppressWarnings(predict(lm_fit, newdata=test_data))
  }
  cbind(test_data$cellline, rowMeans(predictions))
}
...
#                    'id' = scoring$id, 
#                    'value' = NA)
# for(i in seq_along(bagged$drug)){
parameters <- bagging(training_data, test_data, unique(training_data$drug)[1])
# }
...
bagged <- training_data %>%
            select(-cellline) %>%
            group_by(drug) %>%
            nest() %>%
            mutate(model = map(data, ~ bagging(response ~ ., data = ., nbagg=1000)))
...
for(i in seq_along(bagged$drug)){
  for(j in seq_along(test_data$cellline)){
    ...
    outcome$value[row] <- predict(bagged$model[[i]], test_data[j,])
  }
}

write_csv(outcome[,3:4], 'submission.csv')
